      ...
      {zoom && (
        <div className="fixed inset-0 bg-black/70 z-50 flex items-center justify-center p-6" onClick={() => setZoom(null)}>
          <div className="bg-white rounded-2xl p-6 max-w-sm w-full text-center" onClick={e => e.stopPropagation()}>
            <ZoomIn size={40} className="mx-auto mb-3" color={COLORS.primary} />
            <p className="font-semibold text-[#1B2E1B]">{zoom.certificateName}</p>
            <p className="text-xs opacity-60 mt-1">{zoom.studentName} · {zoom.courseName}</p>
            <p className="text-[11px] opacity-50 mt-3">Preview shown here in production would render the uploaded PDF/image full-size with pinch-zoom.</p>
