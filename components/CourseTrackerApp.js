...
const T = {
  en: {
    ...
    approvedCerts: "Approved Certificates", noApproved: "No approved certificates yet.",
    deleteCert: "Delete", deleteCertConfirm: "Delete this approved certificate? This cannot be undone — it will be permanently removed and will no longer count for this student.",
  ...
  ar: {
    ...
    approvedCerts: "الشهادات المعتمدة", noApproved: "لا توجد شهادات معتمدة حتى الآن.",
    deleteCert: "حذف", deleteCertConfirm: "هل تريد حذف هذه الشهادة المعتمدة؟ لا يمكن التراجع عن هذا الإجراء — ستُحذف نهائيًا ولن تُحتسب لهذا الطالب بعد الآن.",
    deleteCertDone: "تم حذف الشهادة.",
  }
};
      ...
      <div className="max-w-7xl mx-auto px-4 py-6 fade-up" key={view}>
        ...
        {view === "admin" && isAdmin && (
          <AdminView t={t} dark={dark} data={data} addItem={addItem} removeItem={removeItem} addCourse={addCourse} removeCourse={removeCourse}
            submissions={data.submissions} onDeleteSubmission={deleteSubmission} showToast={showToast} lang={lang} />
        )}
      </div>
...
function AdminView({ t, dark, data, addItem, removeItem, addCourse, removeCourse, submissions, onDeleteSubmission, showToast, lang }) {
  ...
  const doDelete = async (id) => {
    ...
    setConfirmId(null);
    showToast?.(res.ok ? t.deleteCertDone : (res.message || "Error"), res.ok ? "success" : "error");
  };
  ...
  return (
    <div>
      ...
      <div className="grid md:grid-cols-2 gap-5">
        ...
        <GlassCard dark={dark} className="md:col-span-2">
          <p className="font-semibold mb-3 flex items-center gap-2"><CheckCircle2 size={16} color={COLORS.success} /> {t.approvedCerts}</p>
          {approved.length === 0 ? (
            <p className="text-xs opacity-50 py-2">{t.noApproved}</p>
          ) : (
            <div className="space-y-1.5 max-h-72 overflow-y-auto">
              {approved.map(s => (
                <div key={s.id} className="flex items-center justify-between gap-2 px-3 py-2 rounded-lg text-sm" style={{ background: dark ? "rgba(255,255,255,0.04)" : "rgba(46,125,50,0.05)" }}>
                  <div className="min-w-0">
                    <p className="truncate font-medium">{s.studentName} <span className="opacity-50 font-normal">· {s.phone}</span></p>
                    <p className="truncate text-xs opacity-60">{s.courseName} — {s.university}</p>
                  </div>
                  {confirmId === s.id ? (
                    <div className="flex items-center gap-1.5 shrink-0">
                      <span className="text-[11px] max-w-[140px]" style={{ color: COLORS.danger }}>{t.deleteCertConfirm}</span>
                      <button onClick={() => doDelete(s.id)} className="px-2.5 py-1.5 rounded-lg text-xs font-semibold text-white shrink-0" style={{ background: COLORS.danger }}>{t.deleteCert}</button>
                      <button onClick={() => setConfirmId(null)} className="px-2.5 py-1.5 rounded-lg text-xs font-semibold shrink-0" style={{ background: dark ? "rgba(255,255,255,0.08)" : "rgba(0,0,0,0.06)" }}>{t.cancel}</button>
                    </div>
                  ) : (
                    <button onClick={() => setConfirmId(s.id)} className="flex items-center gap-1 px-2.5 py-1.5 rounded-lg text-xs font-semibold shrink-0"
                      style={{ color: COLORS.danger, background: dark ? "rgba(255,255,255,0.05)" : "rgba(211,47,47,0.08)" }}>
                      <Trash2 size={13} /> {t.deleteCert}
                    </button>
                  )}
                </div>
              ))}
